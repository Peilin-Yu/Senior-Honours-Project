...
Plots percolation susceptibility chi vs density at a fixed temperature,
scanning across all density folders (each folder = one volume/density point).
Density is computed from the cell volume in the CASTEP .md file.

Usage:  python plot_isoT.py [temperature_K]
...
TEMP_K    = int(sys.argv[1]) if len(sys.argv) > 1 else 400
...
def collect(scan_root, temp_k):
    ...
    for dens in density_dirs:
        ...
        print(f"  {dens}/{temp_k}K ", end="", flush=True)
        ...
        if n_eq < MIN_EQ_FRAMES:
            print(f"[skip] only {n_eq} eq frames")
            continue
        ...
        print(f"chi={chi_mid:.4f}  S={s_mid:.4f}  rho={density_gcc:.4f} g/cc  V={avg_V:.2f} A^3")

        rows.append({
            "density_label": int(dens),
    ...
    return sorted(rows, key=lambda r: r["density_gcc"])
...
def plot(rows, temp_k):
    rho    = [r["density_gcc"]   for r in rows]
    ...
    dens   = [r["density_label"] for r in rows]

    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 5))
    cutoff_label = f"Cutoff = {CUTOFF_MEAN:.3f} +/- {CUTOFF_SE:.3f} A"

    # ── Left panel: susceptibility (chi) — shows a peak ──────────────────────
    ax1.fill_between(rho, chi_lo, chi_hi, alpha=0.2, color="steelblue",
                     label="Cutoff uncertainty")
    ax1.plot(rho, chi, marker="o", color="steelblue", linewidth=2,
             label=cutoff_label)
    for r, c, d in zip(rho, chi, dens):
        ax1.annotate(str(d), (r, c), textcoords="offset points",
                     xytext=(4, 5), fontsize=8, color="grey")
    ax1.set_xlabel("Density (g/cc)", fontsize=13)
    ax1.set_ylabel(r"Susceptibility $\chi$ = std($S_{max}$ / N)", fontsize=13)
    ax1.set_title(f"Susceptibility vs Density  (T = {temp_k} K)", fontsize=12)
    ax1.legend(fontsize=9)
    ax1.grid(True, alpha=0.4)

    # ── Right panel: mean largest cluster size — shows a sigmoidal step ──────
    ax2.fill_between(rho, s_lo, s_hi, alpha=0.2, color="crimson",
                     label="Cutoff uncertainty")
    ax2.plot(rho, s, marker="o", color="crimson", linewidth=2,
             label=cutoff_label)
    for r, sv, d in zip(rho, s, dens):
        ax2.annotate(str(d), (r, sv), textcoords="offset points",
                     xytext=(4, 5), fontsize=8, color="grey")
    ax2.set_xlabel("Density (g/cc)", fontsize=13)
    ax2.set_ylabel(r"Mean largest cluster  $\langle S_{max} / N \rangle$",
                   fontsize=13)
    ax2.set_title(f"Largest cluster size vs Density  (T = {temp_k} K)",
                  fontsize=12)
    ax2.legend(fontsize=9)
    ax2.grid(True, alpha=0.4)

    plt.suptitle(f"H-bond percolation  (T = {temp_k} K)  --  "
                 f"equilibrated frames only (2nd half)",
                 fontsize=13, y=1.01)
    plt.tight_layout()

    fname = f"chi_vs_rho_{temp_k}K.png"
...
if __name__ == "__main__":
    ...
    rows = collect(SCAN_ROOT, TEMP_K)

    if not rows:
        print("No data collected.")
        sys.exit(1)

    print(f"\nCollected {len(rows)} density points.\n")
    plot(rows, TEMP_K)
